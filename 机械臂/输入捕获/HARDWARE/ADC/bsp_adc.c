...
static void ADCx_Mode_Config(void)
{
	...
	DMA_ITConfig(ADC_DMA_CHANNEL, DMA_IT_HT, ENABLE);
	
	// 使能 DMA 通道
	DMA_Cmd(ADC_DMA_CHANNEL , ENABLE);
	
	NVIC_InitStructure.NVIC_IRQChannel = DMA1_Channel1_IRQn;  //TIM2中断
	...
	ADC_RegularChannelConfig(ADC_x, ADC_CHANNEL3, 1, ADC_SampleTime_55Cycles5);
...
void DMA1_Channel1_IRQHandler(void)
 {
   if(DMA_GetITStatus(DMA1_IT_HT1))
   {
		...
		Curren.Roll = ADC_ConvertedValue*360.0/4096.0;
